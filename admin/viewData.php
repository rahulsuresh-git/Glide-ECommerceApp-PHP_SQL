<?php
session_start();
$ip = getenv('HTTP_CLIENT_IP') ?:
getenv('HTTP_X_FORWARDED_FOR') ?:
getenv('HTTP_X_FORWARDED') ?:
getenv('HTTP_FORWARDED_FOR') ?:
getenv('HTTP_FORWARDED') ?:
getenv('REMOTE_ADDR');
date_default_timezone_set('Asia/Kolkata');

$today = date('l');
include 'db.php';

?>
<!DOCTYPE html>
<!--[if lte IE 6]><html class="preIE7 preIE8 preIE9"><![endif]-->
<!--[if IE 7]><html class="preIE8 preIE9"><![endif]-->
<!--[if IE 8]><html class="preIE9"><![endif]-->
<!--[if gte IE 9]><!--><html><!--<![endif]-->
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width,initial-scale=1" />
    <title>Glide - The E-Commerce Portal</title>
    <meta name="author" content="name" />
    <meta name="description" content="description here" />
    <meta name="keywords" content="keywords,here" />
    <script
      src="https://code.jquery.com/jquery-3.3.1.min.js"
      integrity="sha256-FgpCb/KJQlLNfOu91ta32o/NMZxltwRo8QtmkMRdAu8="
      crossorigin="anonymous"
    ></script>
    <link
      rel="shortcut icon"
      href="favicon.ico"
      type="image/vnd.microsoft.icon"
    />
    <link rel="stylesheet" href="css/materialize.min.css" type="text/css" />
    <script src="js/materialize.min.js"></script>
    <link
      href="https://fonts.googleapis.com/icon?family=Material+Icons"
      rel="stylesheet"
    />

    <style>
      body {
        background-color: ghostwhite;
      }
      .card-image {
        height: 150px; /* Your height here */
        overflow: hidden;
      }
      .btn-large {
        border-radius: 40px !important;
        width: 200px;
        margin: 10px;
      }
      .card-content {
        height: 100px;
      }
      span {
        color: #000;
      }
    </style>
  </head>
  <body>
    <div class="row">
    <div class="navbar-fixed">

      <nav>
        <div class="blue nav-wrapper">
          <a href="#" data-target="slide-out" class="sidenav-trigger"
            ><i class="material-icons">menu</i></a
          >

          <a class="brand-logo center" style="font-size:20px">Customer Data</a>
        </div>
      </nav>
      </div>
      <ul id="slide-out" class="sidenav">
        <li>
          <div class="user-view">
            <div class="background">
              <img
                src="images/bg.jpg"
              />
            </div>
            <a href="#user"
              ><img
                class="circle"
                src="data:image/png;base64,"
            /></a>
           <a > <span class="white-text name"><?php echo "ADMIN" ?></span></a>
            <span class="white-text email" style="margin-bottom:20px;padding:0">
              <?php echo "ACCOUNT" ?></span></a>

          </div>
        </li>
        <li>
            <a href="home.php"><i class="material-icons">restaurant_menu </i>Home</a>
          </li>
          <li><div class="divider"></div></li>
          <li id="placed">
            <a   href="viewData.php"><i class="material-icons">shopping_cart
</i>View/Edit Customer Data</a>
          </li>
          <li id="placed">
            <a   href="viewAll.php"><i class="material-icons">shop</i>View All Orders</a>
          </li>

      </ul>
    </div>

    <div class="row">
    <div class="container">
    <form id="myform1" action="viewData1.php" method="post">
    <div class="input-field col s12">
          <input id="last_name" type="number" name="id" class="validate" required >
          <label for="last_name" >Enter FITLITE ID Number</label>
          <button type="submit" class="sub waves-effect black btn btn-success" style="" name="complete" value="SEARCH">SEARCH</button>

        </div></form>
        <div id="myTable"></div>

</div>


   </div>
   </body>
<script>
    $(document).ready(function() {
      $(".sidenav").sidenav();
    });</script>
<script>
$("#myform1").submit(function(e) {
      e.preventDefault();
var url='viewData1.php';
      var form = $(this);
          $.ajax({
        type: "POST",
        url: url,
        data:form.serialize(),
        // serializes the form's elements.
        success: function(data) {
            if(data.includes("wrong"))
            {
                document.getElementById("myTable").innerHTML="<p style='color:red;font-weight:bold'>User not found!</p>";

            }
            var obj = JSON.parse(data);

console.log(obj[0]);
document.getElementById("myTable").innerHTML="<form method='post' action='editAdmin.php' id='myForm2'><input type='hidden' name='uid' value="+obj[0].uid+" id='hiddenfield' />"+
"<p>Customer Name: "+obj[0].name+" <input type='submit' class='sub waves-effect  red btn btn-success z-depth-0' name='editName' value='EDIT'></p>"+
"<p>Email: "+obj[0].email+"  <input type='submit' class='sub waves-effect  red btn btn-success z-depth-0' name='editEmail' value='EDIT'></p><p>Address: "+obj[0].addr+" <input type='submit' class='sub waves-effect  red btn btn-success z-depth-0' name='editContact' value='EDIT'></p><p>Contact: "+obj[0].contact+" <input type='submit' class='sub waves-effect  red btn btn-success z-depth-0' name='editContact' value='EDIT'></p><p>Balance: "+obj[0].balance+" <input type='submit' class='sub waves-effect  red btn btn-success z-depth-0' name='editBalance' value='EDIT'></p><p>Rewards: "+obj[0].rewards+" <input type='submit' class='sub waves-effect  red btn btn-success z-depth-0' name='editRewards' value='EDIT'></p><p>Litres: "+obj[0].litres+"<input type='submit' class='sub waves-effect  red btn btn-success z-depth-0' name='editLitres' value='EDIT'></p><p>Category: "+obj[0].category+"<input type='submit' class='sub waves-effect  red btn btn-success z-depth-0' name='editCategory' value='EDIT'></p><p>Referral Code: "+obj[0].ref+"</p><p>Litre Flag: "+obj[0].lflag+"</p>";
        }
          });});


//while running this code the template will be appended in your div with json data
    </script>
</html>
